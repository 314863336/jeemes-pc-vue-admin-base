<template>
  <div class="app-container">
    <div class="filter-container">
      <el-form v-if="isMainSearch" :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <el-form-item label="分享人">
          <el-input v-model="searchForm.fromUserName" :readonly="true">
    ...
    >
      ...
      <el-table-column prop="fromUserName" :show-overflow-tooltip="true" label="分享人" align="left" />
      ...
      <el-table-column prop="actions" label="操作" width="300" align="left">
        <template slot-scope="scope">
          ...
          <el-button v-show="isMainSearch" type="danger" size="mini" title="删除" @click="deleteReceived(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
import { toMeList, pageByParentId, deleteReceived } from '@/api/file/received'
...
export default {
  ...
  methods: {
    ...
    findTableData() {
      ...
      toMeList(searchParams).then(res => {
    ...
    deleteReceived(row) {
      this.$confirm('确定删除选择的数据？', '提示', {
        ...
        deleteReceived({ id: row.id }).then(res => {
          this.$message({
            type: 'success',
            message: '分享文件删除成功!'
          })
    ...
    setChoosedUser(user) {
      this.searchForm.fromUserId = user.id
      this.searchForm.fromUserName = user.userName
    }
  }
}
</script>
