...
export default {
  ...
  computed: {
    ...
    isChoosed() {
      return (row) => {
        const data = this.choosedUser.filter(o => o.id === row.id)
      ...
      return () => {
        ...
        if (this.tableData.length === 0) {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            ...
            const data = this.choosedUser.filter(o => o.id === element.id)
            if (data.length === 0) {
              isAll = false
              break
            }
  ...
  methods: {
    ...
    unSelectAllUser() {
      ...
      for (let i = 0; i < this.choosedUser.length; i++) {
        ...
        const data = this.tableData.filter(o => o.id === element.id)
        if (data.length === 0) {
          d.push({
            id: element.id,
            userName: element.userName
    ...
    removeUser(user) {
      for (let i = 0; i < this.choosedUser.length; i++) {
        if (user.id === this.choosedUser[i].id) {
