  ...
  methods: {
    ...
    addMapEvent() {
      this.map.on('click', event => {
        this.addStarMarker(event.coordinate)
        this.$emit('changeCoordinate', event.coordinate)
      })
    },
    /**
     * 显示蓝区边界
    ...
    addStarMarker(coordinates) {
      this.map.removeLayer(this.starLayer)
      ...
      this.starLayer = new VectorLayer({
        zIndex: 5,
        source: new VectorSource({
          features: [feature]
        })
      })
      this.map.addLayer(this.starLayer)
    }
  }
}
</script>
